...
def load_data( fname ):
    attrs = []
    cnt = 0
    cmap = set([1, 4, 5, 6, 7, 8, 11, 13, 14, 15, 16])
...
attrs = load_data( 'data/train_features_sampling.csv' )
...
fo = open( 'car.txt', 'w' )
ln = 0
for l in open( 'data/train_features_sampling.csv' ):
    ln = ln + 1
    if ln == 1:
        continue
    l.strip('\n')
...
ln = 0
for l in open( 'data/test_features_sampling.csv' ):
    ln = ln + 1
    if ln == 1:
        continue
    l.strip('\n')
